...
public class CountDownTimerTest {
	...
	/******
	 * A JUnit class that tests all the associated methods in CountDownTimer
	 * multiple times under different circumstances as well as seeing if exceptions
	 * are properly thrown
	 * 
	 ******/
	...
	// tests that program is throwing constructor exceptions correctly
	...
	// tests that string constructor throws exceptions properly
	@Test(expected = IllegalArgumentException.class)
	public void testStringConstructorWithErrors() {
		new CountDownTimer("89:70:61");
	}

	// tests that add method is throwing exceptions
	@Test(expected = IllegalArgumentException.class)
	...
	@Test(expected = IllegalArgumentException.class)
	public void testAddMethodWithErrors2() {
		CountDownTimer s1 = new CountDownTimer(3, 3, 3);
		CountDownTimer s2 = new CountDownTimer(-3, -3, -3);
		s1.add(s2);
	}

	// tests that sub method is throwing exceptions
	@Test(expected = IllegalArgumentException.class)
	...
	@Test(expected = IllegalArgumentException.class)
	public void testSubMethodWithErrors2() {
		CountDownTimer s1 = new CountDownTimer(3, 3, 3);
		CountDownTimer s2 = new CountDownTimer(-3, -3, -3);
		s1.sub(s2);
	}

	// tests that equals method is throwing exceptions
	@Test(expected = IllegalArgumentException.class)
	public void testEqualsMethodWithErrors1() {
		CountDownTimer s1 = new CountDownTimer(200, 3, 3);
	...
	public void testEqualsMethodWithErrors2() {
		CountDownTimer s1 = new CountDownTimer(25, 212, 999);
		CountDownTimer s2 = new CountDownTimer(s1);
		s2.equals(s1);
	}

	// tests that compareTo method is throwing exceptions
	@Test(expected = IllegalArgumentException.class)
